...
courseRoutes.route('/').get(async function (req, res) {
    ...
    Object.keys(query).length > 1
        ? (delete query[stringQuery], filtSort(query))
    ...
    function filtSort(query) {
        ...
        if (query.content) {
            content.content = { $regex: RegExp(query.content), $options: '-i' };
        }
        if (query.learning_goal) {
            content.learning_goal = { $regex: RegExp(query.learning_goal), $options: '-i' };
        }
    ...
    const courses = await Course.find(content, function (err, courses) {}).catch((err) =>
        console.log(err)
    );
    ...
    Course.paginate(content, { page: pages, limit: lim, sort: { [sorting]: [order] } })
...
courseRoutes.put('/:course_code', (req, res) => {
    ...
    if (req.body.review && 6 > difficulty > 0) {
        ...
        Course.findOneAndUpdate(course, {
            $push: { reviews: review, difficulty: parseInt(req.body.difficulty) }
        })
